...
<body>
	...
	<div class="container" style="padding-top: 50px">
		<table class="table table-bordered table-hover">
			<thead>
				<tr>
					<th scope="col">글쓰기번호</th>
					<th scope="col">이름</th>
					<th scope="col">제목</th>
					<th scope="col">내용</th>
					<th scope="col">등록개시일</th>
					<th scope="col">조회수</th>
				</tr>
			</thead>
			<tbody>
			<c:forEach var="board" items="${Blist}">
				<tr>
					<th scope="row">${board.memberno}</th>
					<td>${board.name}</td>
					<td>${board.title}</td>
					<td>${board.content}</td>
					<td>${board.regtime}</td>
					...
					<td>
					    ...
					    "Bwrite.jsp?num=${board.num}"'>수정</button>
					</td>
					<td><button class="btn btn-danger"
							onClick='location.href=
					"memberDelete.do?member=${board.memberno}"'>삭제</button></td>
				</tr>
			</c:forEach>
			</tbody>
		</table>
		<button class="btn btn-primary" onClick="bmove()">글쓰기</button>
	</div>
	<script
