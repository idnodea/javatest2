...
<%
    // 게시글 목록을 가져오는 예제 코드
    List<Board> adminBlist = new ArrayList<>();
...
<div class="container" style="padding-top: 50px">
    <table class="table table-bordered table-hover">
        ...
        <tbody>
        <c:forEach var="board" items="${adminBlist}">
                <tr>
                    <th scope="row">${board.num}</th>
                    <td>${board.name}</td>
                    <td class="view"> <a href="adminBview.do?num=${board.num}">${board.title}</a> </td>
                    <td>${board.content}</td>
                    <td>${board.regtime}</td>
                    <td>${board.hits}</td>
                    <td>
                 	   <button class="btn btn-warning" onClick='location.href="adminBwrite.jsp?num=${board.num}"'>수정</button>
                      <!--   <c:if test="${board.id == sessionScope.MEMBERID || sessionScope.MEMBERID == 'admin'}">
                            
                        </c:if> -->
                    </td>
                    <td>
                 	   <button class="btn btn-danger" onClick='location.href="adminBdelete.do?num=${board.num}"'>삭제</button>
                      <!--  <c:if test="${board.id == sessionScope.MEMBERID || sessionScope.MEMBERID == 'admin'}">
    ...
    <button class="btn btn-primary" onClick="location.href='adminBwrite.jsp'">글쓰기</button>
