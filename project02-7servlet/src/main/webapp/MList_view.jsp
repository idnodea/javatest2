		...
		<table class="table table-bordered table-hover">
			...
			<c:forEach var="member" items="${MList}">
                <tr>
                    ...
                    <td>${member.name}</td>
                    <td>
                        <c:if test="${sessionScope.MEMBERID == member.id || sessionScope.MEMBERID == 'admin'}">
                            <button class="btn btn-warning" onClick='location.href="updateForm.do?memberno=${member.memberno}"'>수정</button>
                        </c:if>
                    </td>
                    <td>
                        <c:if test="${sessionScope.MEMBERID == member.id || sessionScope.MEMBERID == 'admin'}">
                            <button class="btn btn-danger" onClick='location.href="memberDelete.do?memberno=${member.memberno}"'>삭제</button>
                        </c:if>
                    </td>
                </tr>
            </c:forEach>
			 <!--<c:forEach var="member" items="${MList}">
                ...
                <tr>
                    ...
                    <td>${member.name}</td>
                    <td>
                        <% //if(request.getSession().getAttribute("MEMBERID") != null && request.getSession().getAttribute("MEMBERID").equals(member.getId())) { %>
                           <!-- <button class="btn btn-warning" onclick="location.href='updateForm.do?memberno=${member.memberno}'">수정</button> -->
                        <% //} %>
                   <!--   </td>
                    <td> -->
